<cfscript>
settingsData = deserializeJSON( fileRead( ExpandPath( "item_styles.json" ) ) );

itemsObj = new clik.scripts.items(true);

param name="url.test" default="1a";
...
css = itemsObj.css( ".item",  settings );
</cfscript>
...
<head>
	...
	<style>
		...
		.bodytest-5 .list, .bodytest-5a .list {
			padding:20px;
			max-width: 100%;
			--grid-mode:fit;
			--grid-gap:10px;
		...
		.bodytest-4 .list, .bodytest-4a  .list {
			padding:20px;
			max-width: 100%;
			--grid-mode:fixed;
			--grid-gap:30px;
		...
		.item.scheme-box:hover {
			--title-color: #00C0F9;
		}

		.item.scheme-underline .imageWrap {
			padding:0 0 2px 0;
			border-width:0px 0px 8px 0;	
		}

		.item.scheme-underline:hover .imageWrap {
